...
int main(int argc, char *argv[])
{
    ...
    int send_fd, port, cur_fd_index;
    char *p, buf[1024], pktbuf[2048], dev[16];
    ssize_t len;
    int *fds;
    int num_fds, num_packets, samples;
    int num_fds_mask;
    unsigned int seq = 1;
    ...
    if ((num_fds != 1) && (num_fds & (num_fds - 1)))
    {
        printf("max buffers must be a power of 2\n");
        goto usage_error;
    }

    num_fds_mask = num_fds-1;
    ...
    do {
		...
		if (send(fds[cur_fd_index], &seq, len, 0) == -1)
		{
			fprintf(stderr, "send: %s, fd=%d\n", strerror(errno), fds[cur_fd_index]);
			return EXIT_FAILURE;
		}
		++cur_fd_index;
		cur_fd_index &= num_fds_mask;
		++seq;
		++samples;
...
usage_error:
    fprintf(stderr, "Usage: %s <addr> <port> num_fds num_packets\n", argv[0]);
    printf("    num_fds: maximum number of fds to use (must be power of 2)\n");
